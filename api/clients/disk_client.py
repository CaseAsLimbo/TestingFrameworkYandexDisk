...
class DiskClient:
    ...
    def get_info(self, expected_status: HTTPStatus = HTTPStatus.OK, **kwargs) -> Response:
    ...
    def get_meta(self, path: str, 
                 ...
                 **kwargs):
        dict_of_params = {"path": path, "fields": ",".join(fields) if fields is not None else None, "limit": limit, "offset": offset, "preview_crop": str(preview_crop).lower() if preview_crop is not None else None, "preview_size": preview_size, "sort": sort}
    ...
    def post_upload_from_url(self, 
        ...
        dict_of_params = {"path": path, "url": url, "disable_redirects": str(disable_redirects).lower() if disable_redirects is not None else None, "fields": ",".join(fields) if fields is not None else None}
        query = {k: v for k, v in dict_of_params.items() if v is not None}

        link_response = self._client.post(DiskUploadRouter.UPLOAD, params=query, **kwargs)
    ...
    def post_upload_from_url_raw(self, 
                             path: str, 
                             url: str, 
                             disable_redirects: bool | None = None,
                             fields: list[str] | None = None,
    ...
    def put_upload_file(self, path: str, content: bytes, overwrite: bool = True, fields: list[str] | None = None, expected_status: HTTPStatus = HTTPStatus.CREATED, **kwargs) -> Response:
        """
        ...
        """
        dict_of_params = {"path": path, "overwrite": str(overwrite).lower(), "fields": ",".join(fields) if fields is not None else None}
        query = {k: v for k, v in dict_of_params.items() if v is not None}

        response = self._client.get(
            DiskUploadRouter.UPLOAD, 
            params=query, 
            **kwargs
        )
    ...
    def publish(
        ...
        **kwargs,
        ...
        dict_of_params = {"path": path, "fields": ",".join(fields) if fields is not None else None}
        query = {k: v for k, v in dict_of_params.items() if v is not None}
        json_body = {"public_settings": public_settings if public_settings is not None else {}} 
        response = self._client.put(DiskUploadRouter.PUBLISH, params=query, json=json_body, **kwargs)
        self._expected(response, expected_status)
        return response
    ...
    def delete(
        ...
        **kwargs,
        ...
        query = {k: v for k, v in dict_of_params.items() if v is not None}
        response = self._client.delete(DiskUploadRouter.RESOURCES, params=query, **kwargs)
        self._expected(response, expected_status)
        return response   
    ...
    def _wait_polling(self, href: str, expected_status: HTTPStatus = HTTPStatus.OK, poll_interval: float = 1.0, timeout: float = 120.0) -> None:
        start = time.time()
