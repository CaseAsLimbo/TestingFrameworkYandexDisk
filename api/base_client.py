...
class BaseClient:
    def __init__(
        ...
        if oauth_token is not None:
            self._session.headers[
                "Authorization"
            ] = f"OAuth {oauth_token}"  # отключать выборочно для публичных методов - большинство требуют авторизации, поэтому вкючаем по умолчанию.
    ...
    @retry(
        ...
        | retry_if_result(retryable_status),
    )
